...
import { LayoutDashboard, ShoppingBag, PlusCircle, Users, Settings, Package, Bell, Search } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="p-4 border-t border-gray-100">
          <Link to="/" className="text-xs text-gray-400 hover:text-maroon font-poppins">
            ← Back to Storefront
          </Link>
        </div>
      </div>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-4 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
                  <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                    <h3 className="text-xs font-black uppercase tracking-widest text-gray-500">New Bookings</h3>
                    ...
                    {pendingOrders.length > 0 ? (
                      pendingOrders.map((order) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-800">New Rental from {order.user?.name || 'Customer'}</p>
                            <p className="text-[10px] text-gray-400 font-mono">Order ID: #{order._id.slice(-6).toUpperCase()}</p>
                          </div>
                        </Link>
                      ))
                    ) : (
                      <div className="p-10 text-center">
                        <p className="text-sm text-gray-400">Sab sahi hai! Koi nayi booking nahi hai.</p>
                      </div>
                    )}
                  </div>
                  <Link 
                    to="/rani-manager/orders" 
                    onClick={() => setShowNotifications(false)}
                    className="block p-4 text-center text-[10px] font-black uppercase tracking-widest text-maroon hover:bg-gray-50 transition-colors border-t border-gray-100"
                  >
                    View All Bookings
                  </Link>
                </div>
